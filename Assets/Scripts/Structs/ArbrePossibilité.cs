...
public class ArbrePossibilité
{
	...
	public void lireFichier(string texte)
	{
		...
		string[] lines = texte.Split(new string[]{Environment.NewLine}, StringSplitOptions.None);
		//Console.WriteLine (lines[0]);

		foreach (string line in lines) {
			string[] elem = line.Split(new char[1]{';'});
			//Debug.Log ("ligne = "+line);
	
			addArrete (int.Parse(elem[0].ToString()), elem [1], int.Parse(elem[2].ToString()));
	...
	public void afficheArbrePossibilite()
	{
		...
		for (int i = 0; i < this.arbre.Count; i++)
		{
			...
			Debug.Log(idPere + ": " + AfficheListePossibilité(idPere));
		}
	}
}